...
var self = {
        ...
        if(!value.indexOf("__")) {
            ...
            if(match && match.length === 3) {
                ...
                var ops = {
                    ...
                    "range": self.values.rangeValue,
                    "nrange": self.values.rangeValue
                };

                var allowNull = function(op) {
                    switch(op) {
                        case "$elemmatch":
                        case "$lteq":
                        case "$gte":
                ...
                if(_extendedOps && _extendedOps[op] && typeof _extendedOps[op].fn === 'function') {
                    ...
                    var value = operator.fn(value, self, op, fieldName);
                    if(value == null && !allowNull(ns)) {
                        return rtn;
                    }
    ...
    values: {
        ...
        rangeValue: function(value) {
            var result = {};
            value = (value || '').split("_");
            if(value.length >= 1 && !isNaN(value[0] * 1)) {
                result["$gte"] = self.cleanItem(value[0], "range");
            }
            if(value.length >= 2 && !isNaN(value[1] * 1)) {
                result["$lt"] = self.cleanItem(value[1], "range");
            }
            return result;
        }
    }
